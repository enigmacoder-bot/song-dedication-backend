...
const songRequestSchema = new mongoose.Schema({
  ...
  artist: String,
  requestedBy: String,
  requestedByUsername: String, // New field for username
  requestedByEmail: String, // New field for email
  message: String,
...
// Middleware to decode JWT and attach user to socket
io.use((socket, next) => {
...
io.on("connection", (socket) => {
  ...
  socket.on("newRequest", async (data) => {
    try {
      ...
      const request = new SongRequest({
        ...data,
        user: socket.user._id, // Link to the logged-in user
        requestedByUsername: user.username, // Store username
        requestedByEmail: user.email, // Store email
      });
      await request.save();
  ...
  socket.on("markAsCompleted", async (requestId) => {
    try {
      // Update the request to set completed to true
      ...
      if (updatedRequest) {
        io.emit("requestUpdated", updatedRequest); // Broadcast the updated request
      }
    } catch (err) {
...
const createToken = (_id) => {
  return jwt.sign({ _id }, "Song Dedication 222", { expiresIn: "1d" });
};
