...
app.post('/signup', async (req, res) => {
  ...
  app.post('/login', async (req, res) => {
    ...
    //   res.status(200).json({ token });
      res.status(200).json({ redirectUrl: '/dedication' });
    ...
    if(result.length>0){
      ...
      if(expiresAt<Date.now()){
        UserVerification.deleteOne({userId}).then(()=>{
          User.deleteOne({_id:userId}).then().catch((error)=>{
            console.log(error);
          })
        }).catch((error)=>{
          console.log(error);
...
const sendPasswordResetEmail=({_id,email},redirectUrl,res)=>{
  ...
  ResetPassword.deleteMany({userId:_id}).then((result)=>{
    ...
    const mailOptions = {
      ...
      html: `<p>We received a request to reset the password for your account.</p>
              ...
              <p>Click <a href="${redirectUrl+"/"+_id+"/"+resetString}">here</a> to reset your password.</p>
...
app.post('/resetPassword',(req,res)=>{
  ...
  ResetPassword.find({userId}).then(result=>{
    if(result.length>0){
          ...
          if(result){
            ...
            bcrypt.hash(newPassword,10).then(hashedNewPassword=>{
              ...
              .then(()=>{
                ...
                ResetPassword.deleteOne({userId}).then().catch(error=>{
                  console.log(error);
                })
              })
              .catch(error=>{
